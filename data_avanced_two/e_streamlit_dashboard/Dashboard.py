...
response = requests.get(url, params=query_string)  # Requisição da api
dados = pd.DataFrame.from_dict(response.json())    # Transformando em json
dados['Data da Compra'] = pd.to_datetime(          # Convertendo para Datetime
...
with aba1:
    # Para criação de colunas no streamlits precisamos passar tudo em variaveis, assim poderemos mostrar graficos um do lado do outro.
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric('Receita', formata_numero(dados['Preço'].sum(), 'R$'))
        # o use_container_width é para sermpe respeitar o tamanho dos conteiners para evitar sobreposição
